...
SLACK_TOKEN = os.environ["SLACK_TOKEN1"]
CANAL = "C040H7TV6JG"
...
COLECTAS = {
    "Sucursal Monterrey": {"ventana":"15:00-17:00","limite":"10:00 hs"},
    "Sucursal Cd. Mx.":   {"ventana":"14:35-16:35","limite":"07:35 hs"},
    "DEMSA":              {"ventana":"09:15-11:15","limite":"02:15 hs"}
}
DEPOSITO_A_CEDIS = {
    "Monterrey Avenida General Pabl": "Sucursal Monterrey",
    "Iztapalapa Espana":              "Sucursal Cd. Mx.",
    "Iztapalapa España":              "Sucursal Cd. Mx.",
    "DEMSA GDL":                      "DEMSA"
}
CEDIS_CONFIG = [
    ("Sucursal Monterrey", MENCIONES_MTY),
    ("Sucursal Cd. Mx.",   MENCIONES_CDMX),
    ("DEMSA",              MENCIONES_DEMSA)
]
...
if not archivos:
    ...
    slack_post(CANAL_ALERTAS, "⚠️ VitauBot: No hay archivo ML en la carpeta de Drive")
    exit(1)
...
def _get_col(df, exact=None, contains=None):
    """Busca columna por nombre exacto primero, luego por substring. Lanza error claro si no existe."""
    if exact and exact in df.columns:
        return exact
    if contains:
        matches = [c for c in df.columns if contains.lower() in str(c).lower()]
        if matches:
            return matches[0]
    raise ValueError(f"No se encontró columna: exact='{exact}' contains='{contains}' en {list(df.columns)}")
...
print(f"Columnas: id={id_col} | sku={sku_col} | dep={dep_col} | entrega={ent_col} | estado={est_col} | desc={desc_col}")
...
total_etiqueta = len(df)

# Solo órdenes para HOY — excluir las que dicen "colecta que pasará mañana"
df = df[~df[desc_col].astype(str).str.contains("mañana", na=False)]
print(f"ML: {total_etiqueta} etiqueta lista → {len(df)} son para HOY (excluidas {total_etiqueta - len(df)} para mañana)")
...
ids_snapshot     = []
discrepancias    = []
vitau_ids_usados = set()  # evita reusar el mismo vitau en dos órdenes ML distintas
...
for cedis, menciones in CEDIS_CONFIG:
    ...
    for _, row in grp.iterrows():
        ...
        # Fallback: match por SKU sin reutilizar órdenes ya asignadas
        if not v:
            cands = vitau_by_ean.get(row[sku_col], [])
            for cand in cands:
                cand_id = str(cand.get("ID externo", ""))
                if cand_id not in vitau_ids_usados:
                    v = cand
                    break

        # Registrar vitau usado para no reutilizarlo
        if v:
            vitau_ids_usados.add(str(v.get("ID externo", "")))
        ...
        # Marcar órdenes demoradas
        tag_dem = " ⚠️ _DEMORADA_" if any(x in desc.lower() for x in ["demorado", "no fue enviado"]) else ""

        lineas.append(f"🏷️ {orden_str} - {prod} | {st}{tag_dem}")
        ids_snapshot.append(oid)

        if v.get("CEDIS") and v.get("CEDIS") != cedis:
            discrepancias.append({"id": oid, "orden": orden_num, "producto": prod,
                                  "cedis_ml": cedis, "cedis_vitau": v.get("CEDIS")})
...
if discrepancias:
    ...
    slack_post(CANAL_ALERTAS,
               f"🔴 *DISCREPANCIA DE CEDIS*\n{MENCIONES_ALERTAS}\n\n" +
               "\n".join(lineas_d) + f"\n\n_Corte: {hora}_")
